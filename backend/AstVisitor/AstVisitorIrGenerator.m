...
% sort of a "rule book".
    ...
    methods
        ...
        function out = visitAccess(thisVisitor, accessNode)
            ...
            if nChild == 1
                % we are given a branch or a port or a node with reference
                % to ground

                childNode = accessNode.get_child(1);

                if childNode.has_type('var') == true
                    % the argument is a branch label or a node with
                    % reference to ground
                    ...
                    if isempty(branchObj)
                        ...
                        if module.hasNode(childNode.get_name)
                            % this is a node with reference to ground

                            nodeObj1 = module.getNode(childNode.get_name); % this node
                            
                            if ~module.hasGlobalGroundTerminalFor(nodeObj1)
                                % spawn a global ground terminal if have not
                                % done so
                                module.addGlobalGroundTerminalFor(nodeObj1); % add a global ground for nodeObj1
                            end
                            
                            nodeObj2 = module.getGlobalGroundTerminalFor(nodeObj1); % reference node
                            
                            nodeLabel1 = nodeObj1.getLabel;
                            nodeLabel2 = nodeObj2.getLabel;
                            ...
                            pfLabel = [accessLabel, '(', nodeObj1.getLabel, ')'];
                            errMsg = sprintf(['Warning accessing %s. ',...
                                'Node ''%s'' is referencing to ground. A %s ground terminal ''%s'' has been added in ModSpec! Referencing to ground is STRONGLY discouraged!'],...
                                pfLabel,...
                                nodeObj1.getLabel,...
                                nodeObj2.discipline.name,...
                                nodeObj2.getLabel);
                            %                         errorAtAstNode(errMsg, accessNode);
                            %                         error('Exiting IR generator!');
                            
                            VAPP_error('vapp-warning', errMsg, accessNode);
